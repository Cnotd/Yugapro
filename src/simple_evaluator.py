...
class SimpleEvaluator:
    ...
    def _normalize_pose_standards(pose_standards: Dict) -> Dict:
        # 外部传入 None 或错误类型时统一转为空规则，避免后续 in 操作崩溃。
    ...
    def _angle_mapping(self) -> Dict[str, List[str]]:
        """把 MediaPipe 角度字段映射到规则库里的中文角度名。"""
        return {
            "left_knee": ["膝关节", "支撑腿膝关节", "前腿膝关节", "后腿膝关节"],
            "right_knee": ["膝关节", "支撑腿膝关节", "前腿膝关节", "后腿膝关节"],
            "left_hip": ["髋关节", "抬髋关节"],
            "right_hip": ["髋关节", "抬髋关节"],
            "spine_angle": ["脊柱角度", "脊柱延展"],
            "body_tilt": ["侧屈角度"],
            "left_shoulder": ["肩关节", "手臂伸展"],
            "right_shoulder": ["肩关节", "手臂伸展"],
        }
    ...
    def _calculate_accuracy(self, stats: Dict, pose_standards: Dict) -> int:
        ...
        angle_mapping = self._angle_mapping()
            ...
            for standard_name, angle_range in standard_angles.items():
                if standard_name in angle_mapping.get(measured_name, []):
                    ...
                    if measured_value < min_val:
                        problems.append(
                            f"{standard_name}角度过小({measured_value:.1f}°,标准范围{min_val}-{max_val}°)"
                        )
                    elif measured_value > max_val:
                        problems.append(
                            f"{standard_name}角度过大({measured_value:.1f}°,标准范围{min_val}-{max_val}°)"
                        )
                    break
    ...
    def _generate_suggestions(self, problems: List[str], pose_standards: Dict) -> List[str]:
        ...
        if "common_errors" in pose_standards:
            ...
            for problem in problems[:3]:
                if "膝关节" in problem or "腿" in problem:
                    suggestions.append("注意膝盖对齐,保持支撑腿稳定发力")
                elif "髋关节" in problem or "髋部" in problem:
                    suggestions.append("调整髋部位置,保持骨盆展开且不要下沉")
                elif "脊柱" in problem:
                    suggestions.append("保持脊柱延展,避免拱背或塌腰")
                elif "肩关节" in problem or "手臂" in problem:
                    suggestions.append("展开肩膀并主动延伸手臂,保持上下肢对齐")
                elif "角度" in problem:
